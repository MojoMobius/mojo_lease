        ...
        $final_val = array();
                    ...
                    foreach ($mainvalues as $key => $valueloop){
                        ...
                        foreach ($valueloop as $keys => $valueloopkeys){
                            ...
                            $data.=$data_fdrid.'*'.$campaign_name.'*'.$field.'*'.$valueloopkeys.'*'.$html_file_name.'*'.$data_field_type.'*'.$attr_sub_name.'*'.$data_sequence;
                            $data = rtrim($data, '*');
                            $data.= PHP_EOL;
                            }
                            $records_count++; 
                           if($records_count>400000){
                            if (headers_sent()) {
                                echo 'header sent';
                            }
                            while (ob_get_level() && ob_end_clean());
...
//        $HeaderFields = implode('*', $header_fields);
        if (!empty($ExportProduction)) {
//            $data = $HeaderFields."*".$modulename;
//            $data.= PHP_EOL;
                
//            $i = 0;
//            foreach($ExportProduction[0] as $key=>$value) {
//                $values = $ExportProduction[1][$i];
//                $return = array_slice($values, 0, count($values)-1, true);
//                $j=0;
//                foreach($return as $innervalue) {
//                    $ExportProduction[0][$i]['moduleId_'.$j] = $user_list[$innervalue];  
//                    $j++;
//                    $ExportProduction[0][$i]['moduleId_'.$j] = $userGroupnames[$innervalue];  
//                    $j++;
//                }
//                $i++;
//            }
//        
//            for($i=0;$i<count($ExportProduction[0]);$i++) {
//                unset($ExportProduction[0][$i]['InputEntityId']);
//            }
            
            //$userOrderId = $userOrderId-1;
            //$QcuserOrderId = $QcuserOrderId-1;
            //pr($ExportProduction); die;
//            foreach ($ExportProduction as $val) {
//                $HereInputEntityId = $val['InputEntityId'];
//                $i = 1;
//                unset($val['InputEntityId']);
//                unset($val['SequenceNumber']);
//                foreach ($val as $val2) {
//                    
//                    $val2 = $this->char($val2);
//                    //echo $i."------".$userOrderId."-------------<br>";
//                    if (isset($userOrderId) && $i == $userOrderId) {
//                        foreach ($module as $key => $vals) {
//                            if ($moduleConfig[$key]['IsAllowedToDisplay'] == 1) {
//                                $data.= $timeDetails[$key][$HereInputEntityId]['Start_Date']."*";
//                                $data.= $timeDetails[$key][$HereInputEntityId]['End_Date']."*";
//                                $data.= $timeDetails[$key][$HereInputEntityId]['TimeTaken']."*";
//                                $data.= $user_list[$timeDetails[$key][$HereInputEntityId]['UserId']]."*";
//                                $data.= $userGroupnames[$timeDetails[$key][$HereInputEntityId]['UserId']] . "*";
//                            }
//                        }
...
//            }
//            die;
//            foreach ($ExportProduction[0] as $val) {
//                $i = 1;
//                foreach ($val as $val2) {
//
//                    $val2 = $this->char($val2);
...
//                        foreach ($module as $key => $vals) {
//                            if ($moduleConfig[$key]['IsAllowedToDisplay'] == 1) {
//                                $data.=$user_list[$timeDetails[$key][$val['InputEntityId']]['UserId']] . "*";
//                            }
//                        }
//                    } else if (isset($QcuserOrderId) && $i == $QcuserOrderId) {
//                        $data.= $user_list[$val2] . "*";
...
//            if (trim($data) === trim($HeaderFields)) {
//                $this->Flash->error(__('No Record found for this combination!'));
//            } else {
                if (headers_sent()) {
                    echo 'header sent';
