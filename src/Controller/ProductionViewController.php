...
class ProductionViewController extends AppController {
...
//                $this->set('QueryDetails', $QueryDetails[0]);
                ...
                $this->render('/ProductionView/view_vertical');
                /* GRID END******************************************************************************************************************************************************************* */
            } else {
                ...
                $this->render('/ProductionView/view');
            }
        }
...
//        $this->set('Projects', $ProListFinal);
//        if (isset($ProjectId)) {
//            $ProjectId = $this->request->data['ProjectId'] = $ProjectId;
//        }
        ...
        if (isset($this->request->data['ProjectId']) || isset($this->request->data['RegionId'])) {
            $region = $this->ProductionView->find('region', ['ProjectId' => $this->request->data['ProjectId'], 'RegionId' => $this->request->data['RegionId'], 'SetIfOneRow' => 'yes']);
...
//        }
        ...
        if (isset($this->request->data['Allocate'])) {
            $Update = $this->ProductionView->UpdateUserId($this->request->data, $this->Session->read("user_id"));
        ...
        if (isset($this->request->data['check_submit']) || isset($this->request->data['downloadFile'])) {
            ...
            if ($ShowErrorOnly) {
                ...
                //        }
                $ProductionDashboard = $this->ProductionView->find('users', ['condition' => $conditions, 'conditions_timemetric' => $conditions_timemetric, 'Project_Id' => $ProjectId, 'domainId' => $domainId, 'Region_Id' => $RegionId, 'batch_from' => $batch_from, 'batch_to' => $batch_to, 'conditions_status' => $conditions_status, 'UserGroupId' => $UserGroupId, 'UserId' => $user_id]);
                ...
                //$ProductionDashboard = $this->ProductionView->find('users', ['condition' => $conditions, 'Project_Id' => $ProjectId, 'Region_Id' => $RegionId, 'Module_Id' => $ModuleId, 'batch_from' => $batch_from, 'batch_to' => $batch_to, 'conditions_status' => $conditions_status]);
                ...
                if (isset($this->request->data['downloadFile'])) {
                    ...
                    $productionData = '';
                    $productionData = $this->ProductionView->getExportData($Production_dashboard);
                    $this->layout = null;
                    if (headers_sent())
