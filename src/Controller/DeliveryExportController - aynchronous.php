...
//                    $userGroupnames = $this->DeliveryExport->find('getUserGroupName', ['ProjectId' => $ProjectId, 'RegionId' => $RegionId]);
                    //pr($userGroupnames);
                    //exit;
//                    $ExportProductions = $this->DeliveryExport->find('users', ['condition' => $conditions, 'Project_Id' => $ProjectId, 'Region_Id' => $RegionId, 'UserGroupId' => $UserGroupId, 'Module_Id' => $ModuleId, 'batch_from' => $batch_from, 'batch_to' => $batch_to, 'conditions_status' => $conditions_status, 'select_fields' => $select_fields, 'header_fields_vals' => $header_fields_vals, 'select_fields_param' => $select_fields_param, 'prodmodule_fields' => $prodmodule_fields, 'UserId' => $user_id]);
                    $ExportProductions = $this->DeliveryExport->find('users', ['condition' => $conditions, 'Project_Id' => $ProjectId, 'Region_Id' => $RegionId, 'Module_Id' => $ModuleId, 'batch_from' => $batch_from, 'batch_to' => $batch_to, 'conditions_status' => $conditions_status, 'select_fields' => $select_fields, 'header_fields_vals' => $header_fields_vals, 'select_fields_param' => $select_fields_param, 'prodmodule_fields' => $prodmodule_fields]);
...
//                                $valuearr = $this->textarea_slash($valueloopkeys);
        ...
        if (isset($this->request->data['check_view'])) {
            ...
            $file_path = WWW_ROOT . 'uploads/files/'.$batch_from.'/';
            $base_file_path = '/uploads/files/'.$batch_from.'/';
...
//        }
        ...
        if (count($select_fields_param) > 0) {
            ...
            if (!empty($ExportProduction)) {
                if ($records_count < $record_separate_limit) {
                    ...
                    $file_path = WWW_ROOT . 'uploads/files/';
                    $logfile = $file_path . $filename;
                    //$res = $this->asynchdata('https://localhost/mojo_V2/delivery-export/export',array('$filename'=>$filename,'data'=>$data));
                    $url = 'https://localhost/mojo_V2/delivery-export/export';
                    $filecount=1;
                    $current_time = date('m-d-Y-h-i-s');
                    $filename = 'Export_Output_' . $filecount . '_' . $current_time . '.csv';
                    $params = array('$filename'=>$filename,'data'=>$data);
        ...
        fclose($fp);
            ...
            //$this->Flash->error(__('Please add fields in Output Mapping to do export output'));
            echo 'Please add fields in Output Mapping to do export output';
            exit;
        }
...
//        $RegionId = $_POST['regionid'];
//        $date = $_POST['inpdate'];
//            $params=array($ProjectId,$RegionId,$date);
        
        //echo 'siva';
        //$this->render('index');
        //exit;
    }
    function asynchdata($url,$params)
    {
        foreach ($params as $key => &$val) {
          if (is_array($val)) $val = implode(',', $val);
            $post_params[] = $key.'='.urlencode($val);
