        ...
        if (isset($this->request->data['check_submit'])) {
            ...
            if ($ShowErrorOnly) {
                ...
                if (count($select_fields_param) > 0) {
                    ...
                    $queriesR = $ExportProductions[0][6];
                    ...
                    $html_vals = array();
                    $select_fields_exists_group = $this->DeliveryExport->find('getSelectFieldExists', ['condition' => $conditions, 'Project_Id' => $ProjectId, 'Region_Id' => $RegionId, 'UserGroupId' => $UserGroupId, 'Module_Id' => $ModuleId, 'batch_from' => $batch_from, 'batch_to' => $batch_to, 'conditions_status' => $conditions_status, 'select_fields' => $select_fields, 'select_fields_param' => $select_fields_param]);
                    foreach ($queriesR as $key => $values) {
                        ...
                        foreach($specialArr as $key2Tempfdrid=>$value2fdrid){
                            $key2fdrid=  str_replace('_x003', '', $key2Tempfdrid);
                            $key2fdrid=  str_replace('_', '', $key2fdrid);
                            if(is_array($value2fdrid) && count($value2fdrid)==0)
                            {
                                $value2fdrid='';
                            }
                            if (in_array($key2fdrid, $select_fields_exists_group)){
                            if($header_fields_vals[$key2fdrid]=='FDRID'){
                                if(!empty($value2fdrid)){
                                $data_fdrid[$values['InputEntityId']] =$value2fdrid;
                                }
                            }
                            }
                        }
                    ...
                    foreach ($queriesR as $key => $values) {
                        ...
                        $specialArr=  simplexml_load_string($special);
                        $specialArr = json_decode( json_encode($specialArr) , 1);
                        $html_vals = $this->DeliveryExport->find('getHtmlFileName', ['InputEntityId'=>$values['InputEntityId'],'SequenceNumber'=>$values['SequenceNumber'],'condition' => $conditions, 'Project_Id' => $ProjectId, 'Region_Id' => $RegionId, 'Module_Id' => $ModuleId, 'batch_from' => $batch_from, 'batch_to' => $batch_to]);
                        $data_sequence = $values['SequenceNumber'];
                        $data_field_type = $values['FieldTypeName'];
                        foreach($specialArr as $key2Temp=>$value2){
                            ...
                            if (in_array($key2, $select_fields_exists_group)){
                                $html_file_name = $html_vals[$key2][$values['InputEntityId']][$values['DepId']][$values['SequenceNumber']];
                            if ($header_fields_vals[$key2] != 'FDRID') {
                            if(!empty($value2)){
                                $campaign_name = $AttributeGroupMasterName[$key2];
                                    $attr_sub_name = $AttributeSubGroupMasterName[$key2];
                                    $field = $header_fields_vals[$key2];
                                    $valuearr = '"'.$value2.'"';
                                    $data.=$data_fdrid[$values['InputEntityId']] . '*' . $campaign_name . '*' . $field . '*' . $valuearr . '*' . $html_file_name . '*' . $data_field_type . '*' . $attr_sub_name . '*' . $data_sequence;
                                    $data = rtrim($data, '*');
                                    $data.= PHP_EOL; 
                                }
                            }
