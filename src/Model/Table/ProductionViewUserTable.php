...
class ProductionViewUserTable extends Table {
    ...
    public function findRegion(Query $query, array $options) {
        ...
        $template = '';
        $template.='<select name="RegionId" id="RegionId" class="form-control"><option value=0>--Select--</option>';
        if (file_exists($path)) {
    ...
    public function findModule(Query $query, array $options) {
        ...
        if (file_exists($path)) {
            ...
            foreach ($module as $key => $value) {
                $moduleconfig = $contentArr['ModuleConfig'][$key]['IsAllowedToDisplay'];
                if ($moduleconfig == 1) {
                    if ($key == $ModuleId) {
...
//    }
    ...
    public function findUsers(Query $query, array $options) {
        ...
        $UserIdCondition = '';
        if (!empty($UserId))
            $UserIdCondition = " UGMap.UserId = $UserId ";
        ...
        $queriesUGMappingName = $connection->execute("select UGMap.UserId, UGMas.GroupName from MV_UserGroupMapping UGMap"
                ...
                . " where $UserIdCondition AND UGMap.ProjectId = " . $options['Project_Id'] . " AND UGMap.RegionId = " . $options['Region_Id'] . " AND UGMap.RecordStatus = 1 GROUP BY UGMap.UserId, UGMas.GroupName");
...
//                    . " where" . $options['condition'] . " ");
            ...
            if ($test[0] != '') {
                ...
                if (!empty($productionIdarray)) {
                    $connection = ConnectionManager::get('default');
                    //  echo "SELECT InputEntityId,Start_Date,End_Date,TimeTaken,ProductionEntityID,UserId,Module_Id FROM ME_Production_TimeMetric_" . $month . "_" . $year . " WHERE InputEntityId in(" . implode(',', $productionIdarray) . ") $conditions_timemetric";
