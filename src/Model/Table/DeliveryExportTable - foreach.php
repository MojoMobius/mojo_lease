...
//                }
                ...
                if($IsItOkay) {
                    ...
                    $queriesR = $connection->execute("select special,RPE.SequenceNumber,DEP.Id as DepId,DEP.FieldTypeName,RPE.InputEntityId as InputEntityId,PTM.UserId from Report_ProductionEntityMaster_" . $dt . " as RPE INNER JOIN Report_ProductionTimeMetric_" . $dt . " as RPT ON RPE.InputEntityId=RPT.InputEntityId INNER JOIN ME_Production_TimeMetric_" . $dt . " as PTM ON RPE.InputEntityId=PTM.InputEntityId INNER JOIN MC_DependencyTypeMaster as DEP ON DEP.Id=RPE.DependencyTypeMasterId where" . $options['condition'] ." AND UserId in (".$UserIdImploded.") order by InputEntityId,SequenceNumber");
                    $queriesR = $queriesR->fetchAll('assoc');
...
//                        $special='<xml>'.$Production['special'].'</xml>';
...
//                                $value2='';
//                            }
//                            $finalprodValue[$key2][$Production['SequenceNumber']][$Production['DepId']] = $value2;
//                        }
...
//                        }
                    ...
                    foreach ($queriesR as $key => $values){
                        ...
                        //pr($finalprodValue);
                        //exit;
                        $productionIdarray[$prod] = $values['InputEntityId'];
                        $prod++;
                        $queriesHtml = $connection->execute("select AttributeMasterId,HtmlFileName,InputEntityId,DependencyTypeMasterId,SequenceNumber FROM MC_CengageProcessInputData_" . $dt . " where HtmlFileName!='' and InputEntityId = ".$values['InputEntityId']." and SequenceNumber = ".$values['SequenceNumber']);
                        $queriesResHtml = $queriesHtml->fetchAll('assoc');
                        foreach ($queriesResHtml as $HtmlVal){
                            $html_vals[$HtmlVal['AttributeMasterId']][$HtmlVal['InputEntityId']][$HtmlVal['DependencyTypeMasterId']][$HtmlVal['SequenceNumber']]=$HtmlVal['HtmlFileName'];
                        }
                        $special='<xml>'.$values['special'].'</xml>';
                        $specialArr=  simplexml_load_string($special);
                        $specialArr = json_decode( json_encode($specialArr) , 1);
                        ...
                        foreach($specialArr as $key2Temp=>$value2){
                            ...
                            //$finalprodValue[$key2][$values['SequenceNumber']][$values['DepId']] = $value2;
                            if (in_array($key2, $select_fields_exists_group)){
                            if($header_fields_vals[$key2]=='FDRID'){
                                if(!empty($value2)){
                                $fdrid_values[$values['InputEntityId']]=$value2;
                                $main_values[$key][$header_fields_vals[$key2]] = $value2;
                                }
                            }else{
                                if(!empty($value2)){
                                $main_values[$key][$key2] = $value2; 
                                }
                            }
                                $main_values[$key]['InputEntityId'] = $values['InputEntityId'];  
                                $main_values[$key]['UserId'] = $values['UserId'];  
                                $main_values[$key]['DepId'] = $values['DepId'];  
                                $main_values[$key]['SequenceNumber'] = $values['SequenceNumber'];
                                $main_values[$key]['FieldTypeName'] = $values['FieldTypeName'];
...
//        $findArr[2]=$timeDetails;
    ...
    public function findgetSelectFieldExists(Query $query, array $options) {
        $connection = ConnectionManager::get('default');
        $from = strtotime($options['batch_from']);
        $month = date("n", $from);
        $year = date("Y", $from);
	...
	//	echo '<br>';
        ...
        return $select_fields_exists_group;
    }
    public function findgetHtmlFileName(Query $query, array $options) {
        $connection = ConnectionManager::get('default');
        $from = strtotime($options['batch_from']);
        $month = date("n", $from);
        $year = date("Y", $from);
        ...
        $html_vals = array();
            ...
            foreach ($GetPeriodArray as $dt) {
                    ...
                    echo '<pre>';
                        ...
                        $queriesHtml = $connection->execute("select AttributeMasterId,HtmlFileName,InputEntityId,DependencyTypeMasterId,SequenceNumber FROM MC_CengageProcessInputData_" . $dt . " where HtmlFileName!='' and InputEntityId = ".$InputEntityId." and SequenceNumber = ".$SequenceNumber);
                        $queriesResHtml = $queriesHtml->fetchAll('assoc');
                        foreach ($queriesResHtml as $HtmlVal){
                            $html_vals[$HtmlVal['AttributeMasterId']][$HtmlVal['InputEntityId']][$HtmlVal['DependencyTypeMasterId']][$HtmlVal['SequenceNumber']]=$HtmlVal['HtmlFileName'];
                        }
